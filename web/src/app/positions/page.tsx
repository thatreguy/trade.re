...
export default function AllPositionsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
      <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">All Positions - Transparency Dashboard</h1>

      {/* Summary Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
        <div className="bg-trade-card rounded-lg border border-trade-border p-3 sm:p-4">
          <div className="text-gray-500 text-xs sm:text-sm">Total Long</div>
          <div className="text-lg sm:text-xl font-bold text-trade-green">{totalLong.toFixed(2)}</div>
          <div className="text-xs text-gray-500">Avg: {avgLongLeverage.toFixed(1)}x</div>
        </div>
        <div className="bg-trade-card rounded-lg border border-trade-border p-3 sm:p-4">
          <div className="text-gray-500 text-xs sm:text-sm">Total Short</div>
          <div className="text-lg sm:text-xl font-bold text-trade-red">{totalShort.toFixed(2)}</div>
          <div className="text-xs text-gray-500">Avg: {avgShortLeverage.toFixed(1)}x</div>
        </div>
        <div className="bg-trade-card rounded-lg border border-trade-border p-3 sm:p-4">
          <div className="text-gray-500 text-xs sm:text-sm">L/S Ratio</div>
          <div className="text-lg sm:text-xl font-bold">{(totalLong / totalShort).toFixed(2)}</div>
        </div>
        <div className="bg-trade-card rounded-lg border border-trade-border p-3 sm:p-4">
          <div className="text-gray-500 text-xs sm:text-sm">Positions</div>
          <div className="text-lg sm:text-xl font-bold">{positions.length}</div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-4">
        <div className="flex space-x-2">
          <button
            className={`px-3 sm:px-4 py-2 rounded text-sm ${filter === 'all' ? 'bg-white text-black' : 'bg-trade-card text-gray-400'}`}
            onClick={() => setFilter('all')}
          >
            All
          </button>
          <button
            className={`px-3 sm:px-4 py-2 rounded text-sm ${filter === 'long' ? 'bg-trade-green text-white' : 'bg-trade-card text-gray-400'}`}
            onClick={() => setFilter('long')}
          >
            Longs
          </button>
          <button
            className={`px-3 sm:px-4 py-2 rounded text-sm ${filter === 'short' ? 'bg-trade-red text-white' : 'bg-trade-card text-gray-400'}`}
        ...
        <select
          ...
          className="bg-trade-card border border-trade-border rounded px-3 sm:px-4 py-2 text-white text-sm"
        >
      ...
      <div className="sm:hidden space-y-2">
        {filteredPositions.length === 0 ? (
          ...
          filteredPositions.map((pos) => (
            <div key={pos.trader_id} className="bg-trade-card rounded-lg border border-trade-border p-3">
              <div className="flex justify-between items-center mb-2">
                <Link href={`/trader/${pos.trader_id}`} className="font-medium text-sm hover:text-blue-400">
                  {pos.trader_id.slice(0, 8)}...
                </Link>
                <span className={`font-bold ${pos.size > 0 ? 'text-trade-green' : 'text-trade-red'}`}>
                  {pos.size > 0 ? 'LONG' : 'SHORT'}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <span className="text-gray-500">Size:</span> {Math.abs(pos.size).toFixed(3)}
                </div>
                <div>
                  <span className="text-gray-500">Entry:</span> ${pos.entry_price.toFixed(2)}
                </div>
                <div>
                  <span className="text-gray-500">Leverage:</span>{' '}
                  <span className={`px-1 py-0.5 rounded ${getLeverageBadge(pos.leverage)}`}>{pos.leverage}x</span>
                </div>
                <div className={pos.unrealized_pnl >= 0 ? 'text-trade-green' : 'text-trade-red'}>
                  <span className="text-gray-500">P&L:</span> {pos.unrealized_pnl >= 0 ? '+' : ''}${pos.unrealized_pnl.toFixed(2)}
                </div>
              </div>
            </div>
          ))
